...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        std::cout << "Grid1size: " << grid1Size << " Grid2size: " << grid2Size
                  << "\n";
        ...
        std::cout << "Rank:" << rank << " Total Time taken Broadcast(A<-B):"
        ...
        for (Int i = 0; i < iters; ++i)
        {
            ...
            El::copy::TranslateBetweenGridsBroadcast<double, D, D>(
                A,
                B_vector,
                allreduceComm,
                syncGeneral);
        ...
        std::cout << "Rank:" << rank
                  << " Total Time taken Broadcast Comm OPt(A<-B):"
                  << duration_all / (iters - warmup) << endl;
        ...
        for (Int i = 0; i < iters; ++i)
        {
            ...
            {
                *(dynamic_cast<DistMatrix<double, STAR, VC, ELEMENT, D>*>(
                    &(*B_vector[j]))) = A;
            }
#ifdef HYDROGEN_HAVE_GPU
            if (D == Device::GPU)
        ...
        std::cout << "Rank:" << rank << " Total Time taken (A<-B):"
                  << duration_all / (iters - warmup) << endl;
