    ...
    try
    {
        ...
        if (commSize != numSubGrids * grid1_width * grid1_height)
        {
            LogicError("Functionality not supported ",
                       "Number of processes should be equal to the "
                       "numSubGrids*grid1_width*grid1_height");
        }
        ...
        std::cout << "Rank:" << rank
                  << " Total Time taken Scatter Comm OPt(A<-B):"
        ...
        for (Int i = 0; i < iters; ++i)
        {
            ...
            El::copy::TranslateBetweenGridsGather<double, D, D>(
                concat,
                B_vector,
                slice_dim / (numSubGrids * numMatrix),
                allreduceComm,
                syncGeneral1,
                1);
#ifdef HYDROGEN_HAVE_GPU
        ...
        std::cout << "Rank:" << rank << " Total Time taken Gather Comm (A<-B):"
                  << duration_all / (iters - warmup) << endl;
        ...
        std::cout << "Rank:" << rank
                  << " Total Time taken Gathrt Comm OPt(A<-B):"
                  << duration_all / (iters - warmup) << endl;
        ...
        {
            std::cout << "Height: " << concat.LocalHeight()
                      << " and Width: " << concat.LocalWidth() << "\n";
            Print(concat, "Concat");
        }
        ...
        if (test)
        {
            std::cout << "Data Validation Test Passed\n";
        }
        else
        {
            std::cout << "Data Validation Test Failed\n";
        }
    }
